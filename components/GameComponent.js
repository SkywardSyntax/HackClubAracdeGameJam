...
function GameComponent() {
  ...
  return (
    <div className={styles.gameContainer}>
      <div className={styles.astronaut} style={{ left: astronaut.x, top: astronaut.y, borderRadius: '0%' }}></div>
      <div className={styles.blackHole} style={{ left: blackHole.x, top: blackHole.y, width: '200px', height: '200px', backgroundColor: 'gray' }}></div>
      {gold.map((g, index) => (
        <div key={index} className={styles.gold} style={{ left: g.x, top: g.y }}></div>
      ))}
      {debris.map((d, index) => (
        <div key={index} className={styles.debris} style={{ left: d.x, top: d.y, backgroundColor: 'brown' }}></div>
      ))}
    </div>
  );
}
